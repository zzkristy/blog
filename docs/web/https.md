...
大致如图所示。
![传输加密的流程](imgs/tls-ssl-_tcp-ip_protocol.png)
...
**对称加密**的优点在于加密、解密效率通常比较高。缺点在于，数据发送方、数据接收方需要协商、共享同一把密钥，并确保密钥不泄露给其他人。此外，对于多个有数据交换需求的个体，两两之间需要分配并维护一把密钥，这个带来的成本基本是不可接受的。
...
参考下图:
![https加密通信流程](imgs/https_communication.png)
